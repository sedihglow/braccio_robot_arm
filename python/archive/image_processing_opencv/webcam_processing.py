...
class background_buffer:
    ...
    # Calculates a single background frame from what is in the buffer which is
    # used in detecting motion.
    #
    # Calculated a frame which is the result of an average of all frames in the
    # buffer. This limits the effect of small variations between each frame
    # which may have happened due to things out of our control such as lighting
    # from time of day and webcam hardware inconsistancies.
    #
    # This function is used when the buffer is not completely full. When the
    # buffer is completely full update_background is used to obtain the final
    # background frame for the buffer. This is a performance technique to
    # prevent going through each matrix in the buffer every time a new frame is
    # added to the buffer and the background frame is updated.
    ...
    # This function updates the background frame which is the average of the
    # frames in the buffer. The result is the same as calculate_background but
    # only works as intended when the buffer is full and a new frame has been
    # added.
    #
    # This function should not be called if the buffer is not yet full, and
    # should always be called to get the final background frame after it is
    # full.
