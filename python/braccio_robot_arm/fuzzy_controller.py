...
class fuzzy_controller:
    def __init__(self, arduino_serial, kin, cmd, term):
        ...
        self.fuzzy_hand_set.FUZZY_SET[self.HAND_AT]["osv"]  = HAND_AT_OSV
        self.fuzzy_hand_set.FUZZY_SET[self.HAND_NAT]["osv"] = HAND_NAT_OSV

        # init output set values for arm elements
        self.fuzzy_arm_set.FUZZY_SET[self.ARM_FBH]["osv"]  = ARM_FBH_OSV
        self.fuzzy_arm_set.FUZZY_SET[self.ARM_CBH]["osv"]  = ARM_CBH_OSV
        self.fuzzy_arm_set.FUZZY_SET[self.ARM_VCBH]["osv"] = ARM_VCBH_OSV
        self.fuzzy_arm_set.FUZZY_SET[self.ARM_AH]["osv"]   = ARM_AH_OSV
        self.fuzzy_arm_set.FUZZY_SET[self.ARM_VCFH]["osv"] = ARM_VCFH_OSV
        self.fuzzy_arm_set.FUZZY_SET[self.ARM_CFH]["osv"]  = ARM_CFH_OSV
        self.fuzzy_arm_set.FUZZY_SET[self.ARM_FFH]["osv"]  = ARM_FFH_OSV

        # init output set values for base elements
        self.fuzzy_base_set.FUZZY_SET[self.BASE_FL]["osv"] = BASE_FL_OSV
        self.fuzzy_base_set.FUZZY_SET[self.BASE_L]["osv"]  = BASE_L_OSV
        self.fuzzy_base_set.FUZZY_SET[self.BASE_CL]["osv"] = BASE_CL_OSV
        self.fuzzy_base_set.FUZZY_SET[self.BASE_IF]["osv"] = BASE_IF_OSV
        self.fuzzy_base_set.FUZZY_SET[self.BASE_CR]["osv"] = BASE_CR_OSV
        self.fuzzy_base_set.FUZZY_SET[self.BASE_R]["osv"]  = BASE_R_OSV
        self.fuzzy_base_set.FUZZY_SET[self.BASE_FR]["osv"] = BASE_FR_OSV
    ...
    def get_membership(self, fuzzy_set, x_in):
        ...
        if (fuzzy_set is self.fuzzy_hand_set):
            ...
            self.term.eprint("Error: Invalid fuzzy set sent to get_membership")
            return None
            ...
            if (menu_input == EXIT_VAL):
                ...
                if (menu_input == HAND_MENU_IN):
                    ...
                    if (hand_membership):
                        ...
                        self.term.eprint("Error getting hand membership\n")
                    ...
                    if (arm_membership):
                        ...
                        self.term.eprint("Error getting arm membership\n")
                        self.term.wait_for_enter()
                    ...
                    if (base_membership):
                        ...
                        self.term.eprint("Error getting base membership\n")
                        self.term.wait_for_enter()
    ...
    # TODO: Make the ranges printed based on member values in the class so if
    # the range changes in the future for other implementations or other
    # instances of the class it will be accurate.
    ...
    def get_crisp_outputs(self, fuzzy_set):
        ...
        if (fuzzy_set is self.fuzzy_hand_set):
            ...
            self.term.eprint("Error: Invalid fuzzy set to get_crisp_outputs()")
            return None
    ...
    def fuzzy_controller_exec(self):
        ...
        while (stay_flag):
            ...
            print(f"\nhand set crisp output: {hand_output}")
            ...
            #       changing all their angles the same ammount. Need more
            #       testing on physical braccio to see where the motion goes
            ...
            # change the angles on the Braccio
